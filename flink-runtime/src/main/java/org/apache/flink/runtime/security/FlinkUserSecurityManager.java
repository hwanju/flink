...
/**
 * Flink user security manager to control unexpected user behaviors that potentially impact cluster
 * availability, for example, it can warn or prevent user code from terminating JVM by System.exit
 * or halt by logging or throwing an exception. This does not necessarily prevent malicious users
 * who try to tweak security manager on their own, but more for being dependable against user
 * mistakes by gracefully handling them informing users rather than causing silent unavailability.
 */
public class FlinkUserSecurityManager extends SecurityManager {
    ...
    public FlinkUserSecurityManager(CheckExitMode checkExitMode) {
        ...
        LOG.info(
                "FlinkUserSecurityManager is created with {} system exit check (previous security manager is {})",
                this.checkExitMode,
                originalSecurityManager != null ? originalSecurityManager : "not existing");
    }
     ...
     * @return FlinkUserSecurityManager instantiated baesd on configuration. Return null if
     *     disabled.
     */
    public static FlinkUserSecurityManager fromConfiguration(Configuration configuration) {
        final String checkExitModeConfig =
                configuration.getString(SecurityOptions.CHECK_SYSTEM_EXIT);
        ...
        try {
            ...
            throw new IllegalConfigurationException(
                    String.format(
                            "%s is invalid configuration for %s.",
                            checkExitModeConfig, SecurityOptions.CHECK_SYSTEM_EXIT.key()),
                    ex);
        }

        // If no check is enabled, return null so that caller can avoid setting security manager not
